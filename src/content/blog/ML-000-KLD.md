...
Kullback-Leibler Divergence (KL 발산)은 두 확률분포의 차이를 측정하는 양입니다. 엄밀히는 KL 발산은 **두 확률분포 (혹은 확률모델)을 구별하는 로그 증거량의 기댓값**으로 볼 수 있습니다. (증거량이 무엇인지는 뒤에 나옵니다.)

KL 발산을 설명하기 위해, 다음과 같은 약속을 합시다. 같은 표본공간 위에 두 확률분포 $P$와 $Q$가 있다고 합시다. 일단은 두 분포 모두 이산확률분포라고 가정하고, 각각의 확률질량함수를 각각 $p(\cdot)$와 $q(\cdot)$라고 씁시다. 이때 $P$와 $Q$에 대한 KL 발산 $D_{\mathrm{KL}}(P || Q)$은 다음과 같습니다:
...
$$
\log \frac{1+9}{2} = \log 5 \approx 1.609
$$
